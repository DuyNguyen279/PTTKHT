...
public class BillDAO implements DaoInterface<bill> {
    ...
    public int insert(bill t) {
        ...
        try {
            String sql = "INSERT INTO bill (bill_id, created_at, total, tab_id, emp_id, cus_id) VALUES (?, ?, ?, ?, ?, ?)";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setString(1, t.getBill_id());
            ps.setTimestamp(2, java.sql.Timestamp.valueOf(t.getCreated_at().toLocalDate().atStartOfDay()));
    ...
    public bill getTmpBill(String tab_id){
        ...
        Connection conn = JDBCUtil.getConnection();
        try {
            String sql = "SELECT bill_id FROM bill WHERE tab_id = ? AND is_done = 0;";
            PreparedStatement ps = conn.prepareStatement(sql);
    ...
    public void addItem(String bill_id, item i){
        Connection conn = JDBCUtil.getConnection();
        try {
            ...
            ps.executeUpdate();
            sql = "UPDATE bill SET total = total + ? WHERE bill_id = ?;";
            PreparedStatement ps2 = conn.prepareStatement(sql);
            ps2.setDouble(1, i.getItem_price() * i.getItem_quantity());
