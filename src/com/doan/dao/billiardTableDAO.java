...
public class billiardTableDAO implements DaoInterface<billiard_table> {
    ...
    public int insert(billiard_table t) {
        ...
        try {
            String sql = "INSERT INTO billiard_table (tab_id, tab_name, start, end, tab_price, tab_status, is_delete) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    public ArrayList<billiard_table> selectAll() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                billiard_table table = new billiard_table(tab_id, tab_name, start, end, tab_price, status, is_delete);
                list.add(table);
            }
            JDBCUtil.closeConnection(conn);
        } catch (Exception e) {
    ...
    public billiard_table selectById(String id) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                String status = rs.getString("tab_status");
                boolean is_delete = rs.getBoolean("is_delete");
                table = new billiard_table(tab_id, tab_name, start, end, tab_price, status, is_delete);
            }
            JDBCUtil.closeConnection(conn);
        } catch (Exception e) {
            e.printStackTrace();
